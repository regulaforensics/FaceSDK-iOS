...
class DatabaseGroupsViewController: UIViewController {
    ...
    private func showCreateGroupDialog() {
        ...
        alert.addTextField()
      let actionCreate = UIAlertAction(title: "Create".uppercased(), style: .default) { [weak alert, weak self] _ in
    ...
    private func showDeleteGroupDialog(for group: PersonDatabase.PersonGroup) {
        let alert = UIAlertController(title: "Remove PersonGroup", message: "Remove PersonGroup '\(group.name)' from database", preferredStyle: .alert)

      let actionRemove = UIAlertAction(title: "Remove".uppercased(), style: .destructive) { [weak alert, weak self] _ in
    ...
    private func showUpdateGroupDialog(for group: PersonDatabase.PersonGroup) {
        ...
        alert.addTextField()
      let actionCreate = UIAlertAction(title: "Update Person".uppercased(), style: .default) { [weak alert, weak self] _ in
            let name = alert?.textFields?.first?.text
            if let name = name {
                group.name = name
...
extension DatabaseGroupsViewController: UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        let group = groups[indexPath.row]
        let removeAction = UIContextualAction(style: .destructive, title: "Remove group") { [weak self] action, view, completion in
