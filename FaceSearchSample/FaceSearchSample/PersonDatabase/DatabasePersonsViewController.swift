...
class DatabasePersonsViewController: UIViewController {
    ...
    private func showDeletePersonDialog(for person: PersonDatabase.Person) {
        let alert = UIAlertController(title: "Remove Person",
                                      message: "Remove Person '\(group.name)' from database",
                                      preferredStyle: .alert)
        
      let actionCreate = UIAlertAction(title: "Remove".uppercased(), style: .destructive) { [weak alert, weak self] _ in
...
extension DatabasePersonsViewController: UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        let person = persons[indexPath.row]
        let deleteAction = UIContextualAction(style: .destructive, title: "Remove person") { [weak self] action, view, completion in
